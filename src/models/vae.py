...
class SeqStructVAE(nn.Module):
    ...
    def decode_autoregressive(self, z, target_tokens=None, teacher_forcing: bool = True):
        ...
        if teacher_forcing:
            ...
            # 简化：直接用 target_tokens 左移一位，最后一位用 PAD
            inp = target_tokens.clone()
            # 右移一位： [t1,...,tL] -> [PAD, t1,...,tL-1]
            inp[:, 1:] = target_tokens[:, :-1]
            inp[:, 0] = self.pad_id  # 或你想用的 BOS id
            ...
            # causal mask
            # shape = (L,L)，True 代表屏蔽（不看未来）
            tgt_mask = torch.triu(torch.ones(self.max_len, self.max_len, device=device), diagonal=1).bool()
            ...
            # 推理：一步一步生成
            tokens = torch.full((B, self.max_len), self.pad_id, dtype=torch.long, device=device)
            # 我们用第 0 位作为起始 (PAD/BOS)
    ...
    def loss_function(outputs, batch, kl_weight: float = 1e-3, struct_weight: float = 1.0):
        ...
        # ---- 自回归 token 重构 loss ----
        # 我们希望 logits[:, t] 预测的是 true_tokens[:, t]，
        # 但 decoder 输入已经是 shift 的（在 decode_autoregressive 里实现），
        # 所以这里直接用 true_tokens 当 target。
        ...
        # true_contacts 是 0/1，contact_logits 是 (B,L,L)
        # 可选：只对上三角/无对角元素计算；这里简单全矩阵，但可以 mask 掉对角线。
